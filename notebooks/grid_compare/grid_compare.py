...
# =============================================
# Step 1. 전체 df 기준 q_edges 계산
# =============================================
q_codes_all, q_edges = pd.qcut(
    df['pdr_mean'],
    q=10,
    labels=False,
    retbins=True,
...
df['q_idx'] = q_codes_all.astype(float)
nq = len(q_edges) - 1
...
print(f"\n전체 df 기준 q_edges: {nq}개 구간")

# =============================================
# Step 2. df_grid에 q_idx 매핑
# =============================================
...
df_grid['q_idx'] = map_q_idx(df_grid['pdr_mean'].astype(float), q_edges, nq)
print(f"df_grid q_idx 매핑 완료: {len(df_grid)}개")
...
# =============================================
grid_sim = assign_to_grid(grid_meta, df_grid, pdr_col='pdr_mean')
grid_sim = grid_sim.rename(columns={
    'count':      'sim_count',
...
print(f"\n[Diffusion] 로드된 CSV: {len(loaded_samples)}개")

df_gen_all = pd.concat([item['df'] for item in loaded_samples], ignore_index=True)
print(f"[Diffusion] 전체 생성 포인트: {len(df_gen_all)}개")

# =============================================
# Step 6. diffusion 결과 그리드 할당
# =============================================
grid_gen = assign_to_grid(grid_meta, df_gen_all, pdr_col='pdr')
grid_gen = grid_gen.rename(columns={
    'count':      'gen_count',
...
print(f"  RMSE:     {np.sqrt((valid['pdr_diff'] ** 2).mean()):.6f}")
print(f" NRMSE:     {np.sqrt((valid['pdr_diff'] ** 2).mean()) / (valid['sim_mean_pdr'].max() - valid['sim_mean_pdr'].min()):.6f}")
...
# Step 9. 저장
# =============================================
grid_compare.to_csv('./notebooks/grid_compare/grid_compare.csv', index=False)
valid.to_csv('./notebooks/grid_compare/grid_compare_valid.csv', index=False)
print(f"\n저장 완료: grid_compare.csv ({len(grid_compare)}행), ./notebooks/grid_compare/grid_compare_valid.csv ({len(valid)}행)")
