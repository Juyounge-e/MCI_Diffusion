import os
import re
from pathlib import Path

import numpy as np
import pandas as pd
import matplotlib
matplotlib.use("Agg")

import matplotlib.pyplot as plt
import matplotlib.patches as patches
import geopandas as gpd

# =============================================
# 설정
# =============================================
GRID_COMPARE_PATH = r'C:\Users\user00\Desktop\MCI_Diffusion\notebooks\grid_compare\csv\seed_free_compare_rygb_30runs.csv'
GRID_META_PATH    = r'C:\Users\user00\Desktop\MCI_Diffusion\MCI_ADV2\scenarios\daejeon_daejeon_grid\grid_metadata.csv'
DATA_PATH         = Path(r'C:\Users\user00\Desktop\MCI_Diffusion\src\data\daejeon_3000_seed_free_30runs.csv')
SHP_PATH          = r'C:\Users\user00\Desktop\MCI_Diffusion\MCI_ADV2\scenarios\ctprvn.shp'

# 전체 학습 데이터 기준 pdr_q 정보 CSV
PDR_SUMMARY_PATH  = r'C:\Users\user00\Desktop\MCI_Diffusion\notebooks\outputs\analysis\seed_free_30_30runs.csv'

# =============================================
# 데이터 로드
# =============================================
grid_compare = pd.read_csv(GRID_COMPARE_PATH)
grid_meta    = pd.read_csv(GRID_META_PATH)
summary_df   = pd.read_csv(PDR_SUMMARY_PATH)

print(f"grid_compare: {len(grid_compare)}개")
print(f"grid_meta: {len(grid_meta)}개")
print(f"pdr_summary: {len(summary_df)}개")

# =============================================
# Step 1. pdr_summary.csv에서 q_edges 읽기
# =============================================
def parse_interval(s):
    """
    예:
    [0.000025, 0.011383]
    """
    nums = re.findall(
        r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?",
        str(s)
    )

    return float(nums[0]), float(nums[1])


intervals = summary_df["pdr_q"].apply(parse_interval)

left_edges  = [x[0] for x in intervals]
right_edges = [x[1] for x in intervals]

# bins 생성
q_edges = np.array(
    [left_edges[0]] + right_edges,
    dtype=float
)

# unique 사용 X
q_edges.sort()

# 경계 누락 방지
q_edges[0]  = -np.inf
q_edges[-1] = np.inf

nq = len(q_edges) - 1

tick_labels = summary_df["pdr_q"].tolist()

print("\n===== q-bin 정보 =====")
print("q_edges 개수:", len(q_edges))
print("tick_labels 개수:", len(tick_labels))
print("q-bin 개수:", nq)

# =============================================
# pdr -> q_idx 변환 함수
# =============================================
def pdr_to_q_idx(pdr_val, q_edges, nq):

    if pd.isna(pdr_val):
        return np.nan

    idx = np.searchsorted(
        q_edges,
        pdr_val,
        side="right"
    ) - 1

    if idx < 0:
        idx = 0

    if idx >= nq:
        idx = nq - 1

    return float(idx)

# =============================================
# q_idx 생성
# =============================================
grid_compare["sim_q_idx"] = grid_compare["sim_mean_pdr"].apply(
    lambda x: pdr_to_q_idx(x, q_edges, nq)
)

grid_compare["gen_q_idx"] = grid_compare["gen_mean_pdr"].apply(
    lambda x: pdr_to_q_idx(x, q_edges, nq)
)

# =============================================
# grid metadata merge
# =============================================
grid_plot = pd.merge(
    grid_compare,
    grid_meta,
    on="grid_id",
    how="left"
)

print("\nmerge 후 grid_plot:", len(grid_plot))

# =============================================
# 대전 경계 overlay
# =============================================
def overlay_daejeon_boundary(ax):

    try:
        gdf = gpd.read_file(
            SHP_PATH,
            encoding="cp949"
        )

        gdf_wgs84 = (
            gdf
            .set_crs(epsg=5179, allow_override=True)
            .to_crs(epsg=4326)
        )

        gdf_daejeon = gdf_wgs84[
            gdf_wgs84["CTP_ENG_NM"]
            .str.lower() == "daejeon"
        ]

        gdf_daejeon.boundary.plot(
            ax=ax,
            color="black",
            linewidth=1.2,
            linestyle="--"
        )

    except Exception as e:
        print(f"[WARN] boundary skipped: {e}")

# =============================================
# colormap
# =============================================
cmap = plt.colormaps["coolwarm"]

norm = plt.Normalize(
    vmin=0,
    vmax=max(1, nq - 1)
)

# =============================================
# colorbar 추가
# =============================================
def add_colorbar(fig, ax, label):

    sm = plt.cm.ScalarMappable(
        cmap=cmap,
        norm=norm
    )

    sm.set_array([])

    cbar = plt.colorbar(
        sm,
        ax=ax,
        pad=0.02
    )

    cbar.set_label(label)

    cbar.set_ticks(range(nq))
    cbar.set_ticklabels(tick_labels)

    return cbar

# =============================================
# grid heatmap 그리기
# =============================================
def draw_grid_heatmap(
    ax,
    grid_plot,
    q_idx_col,
    title
):

    for _, row in grid_plot.iterrows():

        q_idx = row[q_idx_col]

        # 데이터 없는 셀
        if np.isnan(q_idx):

            facecolor = "lightgray"
            alpha = 0.2

        else:

            facecolor = cmap(norm(q_idx))
            alpha = 0.85

        rect = patches.Rectangle(
            (
                row["bbox_minlon"],
                row["bbox_minlat"]
            ),

            row["bbox_maxlon"] - row["bbox_minlon"],
            row["bbox_maxlat"] - row["bbox_minlat"],

            linewidth=0.2,
            edgecolor="gray",

            facecolor=facecolor,
            alpha=alpha,

            zorder=3,
        )

        ax.add_patch(rect)

    # 대전 경계
    overlay_daejeon_boundary(ax)

    # colorbar
    add_colorbar(
        plt.gcf(),
        ax,
        f"pdr_q bins (from pdr_summary.csv)"
    )

    # 범위
    ax.set_xlim(
        grid_meta["bbox_minlon"].min() - 0.01,
        grid_meta["bbox_maxlon"].max() + 0.01
    )

    ax.set_ylim(
        grid_meta["bbox_minlat"].min() - 0.01,
        grid_meta["bbox_maxlat"].max() + 0.01
    )

    ax.set_title(title)

    ax.set_xlabel("lon")
    ax.set_ylabel("lat")

# =============================================
# Figure 1: Simulation Result
# =============================================
sim_covered = (
    grid_plot["sim_count"] > 0
).sum()

fig1, ax1 = plt.subplots(
    figsize=(11, 9)
)

draw_grid_heatmap(
    ax1,
    grid_plot,
    "sim_q_idx",
    f"Grid Simulation Result "
    f"(covered={sim_covered}/{len(grid_plot)})"
)

plt.tight_layout()

plt.savefig(
    "./notebooks/grid_compare/grid_sim_heatmap.png",
    dpi=150
)

plt.show()

print("저장 완료: grid_sim_heatmap.png")

# =============================================
# Figure 2: Diffusion Result
# =============================================
gen_covered = (
    grid_plot["gen_count"] > 0
).sum()

fig2, ax2 = plt.subplots(
    figsize=(11, 9)
)

draw_grid_heatmap(
    ax2,
    grid_plot,
    "gen_q_idx",
    f"Diffusion Generated Result "
    f"(covered={gen_covered}/{len(grid_plot)})"
)

plt.tight_layout()

plt.savefig(
    "./notebooks/grid_compare/grid_gen_heatmap.png",
    dpi=150
)

plt.show()

print("저장 완료: grid_gen_heatmap.png")
